...
$active_shop_masonry = arum_get_option('active_shop_masonry', 'off');
$shop_masonry_column_type = arum_get_option('shop_masonry_column_type', 'default');
$woocommerce_shop_masonry_columns = arum_get_responsive_columns('woocommerce_shop_masonry_columns');

$product_masonry_container_width = arum_get_option('product_masonry_container_width', 1170);
$product_masonry_item_width = arum_get_option('product_masonry_item_width', 270);
$product_masonry_item_height = arum_get_option('product_masonry_item_height', 450);
$woocommerce_shop_masonry_custom_columns = arum_get_responsive_columns('woocommerce_shop_masonry_custom_columns');
$shop_masonry_item_setting = arum_get_option('shop_masonry_item_setting');


$woocommerce_pagination_type = arum_get_option('woocommerce_pagination_type', 'pagination');

$column_tmp = arum_get_responsive_columns('woocommerce_shop_page_columns');

if($is_main_loop && arum_string_to_bool($active_shop_masonry)){
...
$design = arum_get_option("shop_catalog_grid_style", '1');
...
if($is_main_loop && arum_string_to_bool($active_shop_masonry)){
    arum_set_wc_loop_prop('prods_masonry', true);
    ...
    if( $shop_masonry_column_type != 'custom' ){
        ...
        $loopCssClass[] = arum_get_responsive_column_classes('woocommerce_shop_masonry_columns', array(
            'mobile' => 1,
            'tablet' => 1
        ));
    }
        ...
        arum_set_wc_loop_prop('item_sizes', $__new_item_sizes);
        $masonry_component_type[] = 'AdvancedMasonry';
        $column_tmp = arum_get_responsive_columns('woocommerce_shop_masonry_custom_columns');
    }
    arum_set_wc_loop_prop('image_size', arum_get_option('product_masonry_image_size', 'shop_catalog'));
    ?>
<?php
}
else{
    $loopCssClass[] = 'grid-items';
    $loopCssClass[] = arum_get_responsive_column_classes('woocommerce_shop_page_columns');
}
...
if($is_main_loop){
    ...
    arum_set_wc_loop_prop('pagi_data', array(
