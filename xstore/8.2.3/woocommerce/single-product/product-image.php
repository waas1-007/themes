        ...
        <div class="<?php echo esc_attr( $swiper_wrapper ); ?> main-images">
			...
			if ( $main_attachment_id > 0) {
                ...
                if ( $product_type == 'variation' && get_post_thumbnail_id( $post_id ) < 1) {
	                $html .= get_the_post_thumbnail( $product->get_parent_id(), 'shop_single', $attributes );
                }
                else {
				...
				if ( $main_slider_on ) {
					...
					if ( count( $attachment_ids ) > 0 ) {
						foreach ( $attachment_ids as $key => $attachment_id ) {
							...
							$html = '<div class="' . esc_attr( $gallery_slider ) . ' images woocommerce-product-gallery woocommerce-product-gallery__wrapper"><div data-thumb="' . get_the_post_thumbnail_url( $attachment_id, 'shop_catalog' ) . '" class="' . $et_zoom_class . '"><a href="' . esc_url( $full_size_image[0] ) . '"  data-index="' . $index . '" itemprop="image" class="woocommerce-main-image zoom" >';
