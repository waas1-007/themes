...
if( ! function_exists( 'woodmart_register_required_plugins' ) ) {
	function woodmart_register_required_plugins() {
		...
		if ( $compatible_plugins && ( defined( 'DOING_AJAX' ) || ( isset( $_GET['page'] ) && ( $_GET['page'] === 'tgmpa-install-plugins' && isset( $_GET['plugin'] ) || $_GET['page'] === 'woodmart_dashboard' ) ) ) ) { //phpcs:ignore
			foreach ( $compatible_plugins as $key => $plugin ) {
