...
class Install_Plugins extends Singleton {
	 ...
	 * @since 1.0.0
	 */
	public function get_plugins( $show_plugins = '' ) {
		...
		if ( Setup_Wizard::get_instance()->is_setup() || 'theme_plugin' === $show_plugins ) {
			$xts_plugin = array();
			...
			if ( $plugins ) {
				foreach ( $plugins as $key => $plugin ) {
					...
					$xts_plugin[ $key ] = $plugin;
				}
				$plugins = array_replace( array_flip( $order ), $xts_plugin );
			}
		} elseif ( 'compatible' === $show_plugins ) {
			$xts_plugin = array();
	...
	public function is_all_activated() {
		...
		if ( Setup_Wizard::get_instance()->is_setup() ) {
			$required_plugin = array_merge( $required_plugin, array( 'elementor', 'js_composer' ) );
		} else {
			$required_plugin = array_merge( $required_plugin, array( woodmart_get_current_page_builder() ) );
		}

		foreach ( $plugins as $slug => $plugin ) {
			if ( in_array( $slug, $required_plugin, true ) && ! $tgmpa->is_plugin_active( $slug ) && $tgmpa->can_plugin_activate( $slug ) ) {
				$output[] = $plugin;
			}
		}
