...
if ( isset( $args['show_plugins'] ) ) {
	$plugins_list = $install_plugins->get_plugins( $args['show_plugins'] );
} else {
	$plugins_list = $install_plugins->get_plugins();
}

if ( isset( $args['show_plugins'] ) && 'compatible' === $args['show_plugins'] ) {
	$plugins_list = array_merge( woodmart_get_config( 'compatible-plugins' ), $plugins_list );
}

if ( $dashboard->is_setup() ) {
	$button_item_class = 'xts-inline-btn xts-style-underline';
	$button_all_class  = 'xts-inline-btn xts-style-underline';
} else {
	$button_item_class = 'xts-btn';
	$button_all_class  = 'xts-btn xts-color-alt';
}
...
<div class="xts-plugins<?php echo $install_plugins->is_all_activated() ? ' xts-all-active' : ''; ?>">
	...
	<?php
	if ( isset( $args['show_plugins'] ) ) {
		if ( 'theme_plugin' === $args['show_plugins'] ) {
			$plugins_list = '';
		} elseif ( 'compatible' === $args['show_plugins'] ) {
			$plugins_list = array_merge( $plugins_list, $install_plugins->get_plugins( 'theme_plugin' ) );
		}
	}
	?>
...
<div class="xts-wizard-footer">
	...
	<div>
		<?php if ( ! isset( $args ) ) : ?>
			<a class="<?php echo esc_attr( $button_all_class ); ?> xts-wizard-all-plugins" href="#">
				<?php esc_html_e( 'Install & activate all', 'woodmart' ); ?>
			</a>
		<?php endif; ?>
		<?php if ( $dashboard->is_setup() ) : ?>
			<?php $dashboard->get_next_button( 'dummy-content', '', count( $install_plugins->get_required_plugins_to_activate() ) > 0 ); ?>
		<?php endif; ?>
	</div>
</div>
