...
class Redirect extends Singleton {
	...
	public function get_redirect_url( $product_id ) {
		$url                 = array( 'url' => '' );
		$redirect_in_product = get_post_meta( $product_id, '_woodmart_buy_now_redirect', true );
		$redirect            = $redirect_in_product && 'inherit' !== $redirect_in_product ? $redirect_in_product : woodmart_get_opt( 'buy_now_redirect', 'checkout' );
