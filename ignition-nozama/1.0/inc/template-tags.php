 ...
 * @since 1.0.0
 */
function ignition_nozama_the_header_ajax_search() {
	...
	<div class="head-search-form-wrap">
		<form class="category-search-form <?php echo esc_attr( $ajax_class ); ?>" action="<?php echo esc_url( home_url( '/' ) ); ?>" method="get">
			<label for="category-search-name" class="sr-only" >
				<?php esc_html_e( 'Category name', 'ignition-nozama' ); ?>
			</label>

			<?php
				wp_dropdown_categories( array(
					'taxonomy'          => 'product_cat',
					'show_option_none'  => esc_html__( 'Search all categories', 'ignition-nozama' ),
					'option_none_value' => '',
					'value_field'       => 'slug',
					'hide_empty'        => 1,
					'echo'              => 1,
			...
			<div class="category-search-input-wrap">
				<label for="category-search-input" class="sr-only">
					<?php esc_html_e( 'Search products:', 'ignition-nozama' ); ?>
				</label>
				<input
					type="text"
					class="category-search-input"
					id="category-search-input"
					placeholder="<?php esc_attr_e( 'What are you looking for?', 'ignition-nozama' ); ?>"
					name="s"
						...
						<a href="">
							<?php if ( $show_thumb ) : ?>
								<div class="category-search-results-item-thumb">
									<img src="<?php echo esc_url( wc_placeholder_img_src( ignition_nozama_woocommerce_get_ajax_search_products_thumbnail_size() ) ); ?>" alt="<?php esc_attr_e( 'Search result item thumbnail' ); ?>">
								</div>
							<?php endif; ?>
			...
			<button type="submit" class="category-search-btn">
				<span class="ignition-icons ignition-icons-search"></span><span class="sr-only"><?php echo esc_html_x( 'Search', 'submit button', 'ignition-nozama' ); ?></span>
			</button>
		</form>
	</div>
	<?php
