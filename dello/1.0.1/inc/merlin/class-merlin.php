...
class Merlin {
	...
	protected function server_requirements($step) {
		...
		if ($max_execution_time < 119 && $max_execution_time > 0 ) {
			$max_execution_time = 'error';
			$go = false;
		}
		$upload_max_filesize = intval(substr(size_format( wp_max_upload_size() ), 0, -1));

		if (substr(size_format( wp_max_upload_size() ), -2 ) !="GB") {

			if ($upload_max_filesize < 11 ) {
	...
	?>
		...
		<table class="widefat" cellspacing="0">
			<tbody>
				...
				<?php if ($php_check == 'error'): ?>
					<tr>
						<td><?php esc_html_e( 'PHP Version', 'dello' ); ?></td>
						<td>
							<span class="dashicons dashicons-warning red"></span> <span><?php echo esc_html($phpversion); ?></span>
							<div><small><?php esc_html_e('Recommended: ', 'dello'); ?><strong>7.0 or higher</strong></small></div>
						</td>
					</tr>
				<?php endif; ?>
				<?php	} ?>
				...
				<?php if ($max_input_vars == 'error'):
				 ...
				 ?>
				<tr>
					<td><?php esc_html_e( 'max_input_vars', 'dello' ); ?></td>
					<td>
						<span class="dashicons dashicons-warning <?php echo esc_attr($color); ?>"></span> <span><?php echo ini_get('max_input_vars'); ?></span>
						<div><small><?php esc_html_e('Recommended: ', 'dello'); ?><strong>3000</strong></small></div>
					</td>
				</tr>
				<?php endif; ?>
				...
				<tr>
					<td><?php esc_html_e( 'memory_limit', 'dello' ); ?></td>
					<td>
						<span class="dashicons dashicons-warning red"></span> <span><?php echo ini_get('memory_limit'); ?></span>
						<div><small><?php esc_html_e('Recommended: ', 'dello'); ?><strong>256M</strong></small></div>
					</td>
				</tr>
				<?php endif; ?>
				...
				<tr>
					<td><?php esc_html_e( 'max_execution_time', 'dello' ); ?></td>
					<td>
						<span class="dashicons dashicons-warning red"></span> <span><?php echo ini_get('max_execution_time'); ?></span>
						<div><small><?php esc_html_e('Recommended: ', 'dello'); ?><strong>120</strong></small></div>
					</td>
				</tr>
				<?php endif; ?>
				...
				<tr>
					<td data-export-label="Max Upload Size"><?php esc_html_e( 'Max Upload Size', 'dello' ); ?>:</td>
					<td>
						<span class="dashicons dashicons-warning red"></span> <span><?php echo size_format( wp_max_upload_size() ); ?></span>
						<div><small><?php esc_html_e('Recommended: ' , 'dello'); ?><strong>12M</strong></small></div>
					</td>
				</tr>
				<?php endif; ?>
			...
			<?php endif; ?>
				...
				<?php
					...
					$posting['fsockopen_curl']['help'] = '<a href="#" class="help_tip" data-tip="' . esc_attr__( 'Payment gateways can use cURL to communicate with remote servers to authorize payments, other plugins may also use it when communicating with remote services.', 'dello' ) . '">[?]</a>';
		...
		<?php if ( $go == false) { ?>
			<?php if ( $step == 'welcome' ) : ?>
			<p class="server_check_notice step_welcome red"><?php echo sprintf( __( "Your server does not match the recommended settings to import demo designs properly. Import might fail. You're requested to adjust setting", 'dello' ), '<span class="underline">', '</span>' ); ?></p>
			<p class="server_check_notice step_welcome"><?php echo sprintf( __( '%sAdjust your settings%s, or proceed with caution :(', 'dello' ), '<a href="https://radiantthemes.zendesk.com/hc/en-us/articles/360040991214-Essential-hosting-setting-for-smooth-import-of-demo-designs" target="_blank">', '</a>' ); ?></p>
			<?php else: ?>
			<p class="server_check_notice step_import red"><?php echo sprintf( __( '%sYour server does not match the recommended settings to import demo designs properly. Import might fail.. You are requested to adjust setting.%s ', 'dello' ), '<span class="underline">', '</span>' ); ?></p>
			<?php endif; ?>
		<?php } ?>
