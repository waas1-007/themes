...
if ( !class_exists("Famita_Woo_Brand") ) {
	class Famita_Woo_Brand {

		public static function init() {
			// brand
			$tax = famita_get_config( 'product_brand_attribute' );
			if ( !empty($tax) ) {
		...
		public static function add_brand() {
			...
			<div class="form-field">
				<label><?php esc_html_e( 'Thumbnail', 'famita' ); ?></label>
		...
		public static function edit_brand( $term ) {
			...
			?>
			<tr class="form-field">
				<th scope="row" valign="top"><label><?php esc_html_e( 'Thumbnail', 'famita' ); ?></label></th>
				<td>
		...
		public static function brand_columns( $columns ) {
			...
			foreach ($columns as $key => $value) {
				if ( $key == 'name' ) {
					$new_columns['image'] = esc_html__( 'Image', 'famita' );
				}
		...
		public static function brand_column( $columns, $column, $id ) {
			if ( $column == 'image' ) {
				...
				$columns .= '<img style="max-width: 60px;" src="' . esc_url( $image ) . '" alt="'.esc_attr__( 'Image', 'famita' ).'" class="wp-post-image" />';
			}
		...
		public static function get_product_brands() {
		    global $product;
		    $brands_tax = famita_get_config( 'product_brand_attribute' );
	...
	add_action( 'init', array('Famita_Woo_Brand', 'init') );
}